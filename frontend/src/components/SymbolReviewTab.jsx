...
const SymbolReviewTab = ({ 
    ...
    const startDetection = async () => {
        try {
            ...
            // Refresh runs list
            setTimeout(() => loadDetectionRuns(), 1000);
            
        } catch (err) {
            console.error('Failed to start detection:', err);
            setError('Failed to start detection: ' + (err.response?.data?.error || err.message));
        } finally {
            setIsLoading(false);
        }
    };

    const monitorDetectionProgress = async () => {
        const checkProgress = async () => {
            try {
                const response = await axios.get(`/api/detection_progress/${docInfo.docId}`);

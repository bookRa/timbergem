...
from utils.coordinate_mapping import PDFCoordinates, ImageCoordinates, PageMetadata, CoordinateTransformer
...
@detection_updates_bp.route("/api/update_detection_coordinates", methods=["POST", "OPTIONS"])
def update_detection_coordinates():
    ...
    try:
        ...
        page_metadata = PageMetadata.from_dict(page_metadata_dict["pages"][str(page_num)])
        ...
        detection_storage.update_detection_coordinates(run_id, detection_id, pdf_coords_obj, image_coords)
...
def update_detection_status():
    ...
    try:
        ...
        updates = [{
            "detectionId": detection_id,
            "action": "accept" if new_status == "accepted" else "reject" if new_status == "rejected" else "pending",
...
def add_user_detection():
    ...
    try:
        ...
        page_metadata = PageMetadata.from_dict(page_metadata_dict["pages"][str(page_number)])
        transformer = CoordinateTransformer(page_metadata)
        image_coords = transformer.pdf_to_image(pdf_coords_obj)
        
        # Add user detection
        detection_storage = DetectionStorage(doc_dir)
        detection_id = detection_storage.add_user_detection(run_id, symbol_id, pdf_coords_obj, image_coords, page_number)
...
@detection_updates_bp.route("/api/get_page_image/<doc_id>/<int:page_number>", methods=["GET"])
def get_page_image(doc_id, page_number):
    ...
    try:
        ...
        # Look for page image
        page_image_path = os.path.join(doc_dir, f"page_{page_number}", f"page_{page_number}_pixmap.png")

...
class CoordinateTransformer:
    ...
    def image_to_pdf(self, image_coords: ImageCoordinates) -> PDFCoordinates:
        ...
        pdf_left = image_coords.left * self.image_to_pdf_scale
        pdf_top = image_coords.top * self.image_to_pdf_scale
        pdf_width = image_coords.width * self.image_to_pdf_scale
        pdf_height = image_coords.height * self.image_to_pdf_scale
    ...
    def image_to_canvas(self, image_coords: ImageCoordinates, canvas_width: float, canvas_height: float) -> CanvasCoordinates:
        """
    ...
    def pdf_to_canvas(self, pdf_coords: PDFCoordinates, canvas_width: float, canvas_height: float) -> CanvasCoordinates:
        """
    ...
    def get_canvas_dimensions_for_aspect_ratio(self, max_width: float, max_height: float) -> Tuple[float, float]:
        """
        Calculate optimal canvas dimensions that maintain image aspect ratio.
        Used by frontend to determine proper canvas sizing.
        """
        image_aspect_ratio = self.page_metadata.image_width_pixels / self.page_metadata.image_height_pixels
...
class ClippingCoordinateTransformer:
    ...
    def __init__(self, legend_pdf_coords: PDFCoordinates, clipping_dpi: int, page_metadata: PageMetadata):
        ...
        self.clipping_height_pixels = int(legend_pdf_coords.height * clipping_dpi / 72.0)
        
        # Pre-calculate scale factors
        self.points_per_pixel = 72.0 / clipping_dpi
    
    def symbol_canvas_to_pdf(self, symbol_canvas_coords: CanvasCoordinates) -> PDFCoordinates:
        """
    ...
    def canvas_to_clipping(self, canvas_coords: CanvasCoordinates) -> ClippingCoordinates:
        """
...
def validate_coordinates(coords, coord_type: str) -> bool:
    ...
    """
    if coord_type == "pdf":
        return (coords.width > 0 and coords.height > 0 and 
                coords.left >= 0 and coords.top >= 0)
    elif coord_type == "image":
        return (coords.width > 0 and coords.height > 0 and 
                coords.left >= 0 and coords.top >= 0 and coords.dpi > 0)
    elif coord_type == "canvas":
        return (coords.width > 0 and coords.height > 0 and 
                coords.left >= 0 and coords.top >= 0 and 
                coords.canvas_width > 0 and coords.canvas_height > 0)
    elif coord_type == "clipping":
        return (coords.width_pixels > 0 and coords.height_pixels > 0 and 
                coords.left_pixels >= 0 and coords.top_pixels >= 0 and 
                coords.clipping_dpi > 0)

...
# Resolve repo root and candidate doc roots (support both current and legacy layouts)
...
def list_document_pages(doc_id: str) -> Dict[str, Any]:
    """
    Return an ordered list of pages for the specified document.
    The response shape matches the existing frontend expectation used by ProjectNavigator:
    {
      "docId": "...",
      "pages": [ {"page_number": 1, "sheet_number": "a0.10", "title": "..."}, ... ]
    }
    """
    ...
    # Normalize keys to ints and sort
    ...
    for pn in page_numbers:
        ...
        pages.append(
            {
                "page_number": pn,
                # Optional metadata if present in your pipeline
...
def list_page_entities(doc_id: str, page_number: int) -> Dict[str, Any]:
    """
    Return all knowledge-graph-style entities associated with a single page.
    This consolidates data from existing JSON artifacts if available. Shape:
    {
      "docId": "...",
      "pageNumber": 1,
      "entities": {
    ...
    # Aggregate annotations for the page
    annotations = []
    for a in annotations_data.get("annotations", []):
        if int(a.get("pageNumber", -1)) == page_number:
            annotations.append(a)

    # Summary for page (if present)
    summary_map: Dict[str, Any] = summaries_data.get("summaries", {})
    summary = summary_map.get(str(page_number)) or summary_map.get(page_number)

    # Symbols/Notes/Scopes may be produced by downstream pipelines; attempt to read a consolidated entities file

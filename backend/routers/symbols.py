...
class SymbolDefinitionCreate(BaseModel):
    ...
    source_sheet_number: Optional[str] = Field(None)
    bounding_box: Optional[BoundingBox] = Field(None)
    scope: str = Field(..., description="'project' or specific sheet number like 'a2.10'")
...
def create_symbol_definition(payload: SymbolDefinitionCreate) -> Dict[str, Any]:
    ...
    cypher = (
        ...
        "name: $name, description: $description, source_sheet_number: $source_sheet_number, "
        "scope: $scope, bounding_box: $bounding_box"
        "}) RETURN s"
    )
    params = {
        "name": payload.name,
